...
def fetch_open_meteo(
    ...
    """
    Récupère les données météo courantes via Open-Meteo.
    """
    params = {
        "latitude": latitude,
        "longitude": longitude,
    ...
    try:
        resp = httpx.get(OPEN_METEO_URL, params=params, timeout=20)
        resp.raise_for_status()
    except httpx.HTTPError as exc:
        raise HTTPException(
            503,
            detail=f"Open-Meteo non accessible: {exc}",
        ) from exc
    ...
    meas_list = []
    timestamp = current.get("time") # ISO8601
    ...
    return [{
        "location": f"Open-Meteo ({city_name})",
        "city": city_name,
...
def save_measurements(
    ...
    *,
    ...
    source_url: str = OPENAQ_URL,
    source_description: str = "API qualité de l'air OpenAQ",
):
